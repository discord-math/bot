...
class ChannelState:
    ...
    if TYPE_CHECKING:
        def __init__(self, *, channel_id: int, subscriber: str, last_message_id: int,
            earliest_thread_archive_ts: Optional[datetime] = ...) -> None: ...
...
def take_snapshot(channels: List[Union[TextChannel, VoiceChannel]]) -> Tuple[Dict[int, int], Dict[int, Dict[int, int]]]:
    return ({channel.id: approx_last_msg(channel) for channel in channels},
        {channel.id: {thread.id: approx_last_msg(thread) for thread in channel.threads}
            for channel in channels if isinstance(channel, TextChannel) and len(channel.threads)})
...
async def select_fetch_task(session: AsyncSession, subscribers: Iterable[str]
    ) -> Union[None,
...
async def select_channel_requests_overlapping(session: AsyncSession, subscribers: Iterable[str], channel_id: int,
    before: int) -> Iterable[ChannelRequest]:
    subs = list(subscribers)
    nonrec_first = (select(ChannelRequest.after_snowflake)
    ...
    rec_first = (select(ChannelRequest.after_snowflake)
        .where(ChannelRequest.channel_id == channel_id, ChannelRequest.subscriber.in_(subs),
            ChannelRequest.before_snowflake >= nonrec_first.c.after_snowflake)
        .order_by(ChannelRequest.before_snowflake)
        .limit(1)
        .lateral())
    first = nonrec_first.union(select(rec_first.c.after_snowflake)
        .select_from(nonrec_first.join(rec_first, true())))
    min_after = select(func.min(first.c.after_snowflake)).scalar_subquery()
    stmt = (select(ChannelRequest)
        .where(ChannelRequest.channel_id == channel_id, ChannelRequest.subscriber.in_(subs),
            ChannelRequest.before_snowflake <= before,
            ChannelRequest.after_snowflake >= min_after))
    return (await session.execute(stmt)).scalars()

async def select_thread_requests_overlapping(session: AsyncSession, subscribers: Iterable[str], thread_id: int,
    before: int) -> Iterable[ThreadRequest]:
    subs = list(subscribers)
    nonrec_first = (select(ThreadRequest.after_snowflake)
        ...
        .limit(1)
        .cte("first", recursive=True))
    rec_first = (select(ThreadRequest.after_snowflake)
        .where(ThreadRequest.thread_id == thread_id, ThreadRequest.subscriber.in_(subs),
    ...
    first = nonrec_first.union(select(rec_first.c.after_snowflake)
        .select_from(nonrec_first.join(rec_first, true())))
    min_after = select(func.min(first.c.after_snowflake)).scalar_subquery()
    stmt = (select(ThreadRequest)
        .where(ThreadRequest.thread_id == thread_id, ThreadRequest.subscriber.in_(subs),
            ThreadRequest.before_snowflake <= before,
            ThreadRequest.after_snowflake >= min_after))
    return (await session.execute(stmt)).scalars()

async def select_archive_ts(session:AsyncSession, subscribers: Iterable[str], channel_id: int
...
async def fetch_channel_messages(session: AsyncSession, channel: Union[TextChannel, VoiceChannel],
    ...
    async with sessionmaker() as session:
        ...
        for channel_id in last_msgs:
            if channel_id not in have_chans:
                ...
                if not isinstance(channel, (TextChannel, VoiceChannel)): continue
                subscribers = set()
                for sub in events:
                    subscribers.add(sub)
        ...
        for channel_id in min_last_msgs:
            ...
            async for thread in channel.archived_threads(limit=None):
                ...
                if thread.archive_timestamp < Object(min_last_msgs[channel_id]).created_at: break
        ...
        for state in states:
            ...
            for thread in archived_threads[state.channel_id]:
                if thread.archive_timestamp < Object(state.last_message_id).created_at: continue
...
async def process_message(msg: Message) -> None:
    ...
    async with sessionmaker() as session:
        ...
        stmt = (update(ChannelState)
            .where(ChannelState.channel_id == channel_id, ChannelState.subscriber.in_(list(subscribers)))
            .values(last_message_id = func.greatest(ChannelState.last_message_id,
                literal(msg.id, type_=BigInteger))))
        await session.execute(stmt)
        await session.commit()
    if requests_added:
        update_fetch()
...
async def subscribe(name: str, channels: Optional[Union[Guild, TextChannel, VoiceChannel]], cb: Callback, *,
    missing: bool, retroactive: bool) -> None:
    """
    ...
    if missing or retroactive:
        if channels is None:
            ...
            chans = [channel
                for channel in channels.channels if isinstance(channel, (TextChannel, VoiceChannel))]
        else:
            chans = [channels]
        last_msgs, thread_last_msgs = take_snapshot(chans)

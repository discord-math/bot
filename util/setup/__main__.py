...
async def async_main() -> None:
    async with AsyncSession(util.db.engine) as session:
        ...
        await util.db.init_for("bot.acl", util.db.get_ddl(
            CreateSchema("permissions"),
            bot.acl.registry.metadata.create_all))
        logging.info("Creating schema for bot.commands")
        await util.db.init_for("bot.commands", util.db.get_ddl(
            bot.commands.registry.metadata.create_all))
        logging.info("Creating schema for bot.autoload")
        await util.db.init_for("bot.autoload", util.db.get_ddl(
            bot.autoload.registry.metadata.create_all))
        ...
        admin_id = await asyncio.get_event_loop().run_in_executor(None,
            lambda: int(input("Input your Discord user ID: ")))
        ...
        prefix = await asyncio.get_event_loop().run_in_executor(None,
            lambda: input("Input command prefix: "))

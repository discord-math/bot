...
class AppealView(View):
    def __init__(self, appeal_id: int) -> None:
        ...
        self.add_item(
            Button(style=ButtonStyle.danger, label="Close", custom_id="{}:{}:Close".format(__name__, appeal_id))
        )
...
async def post_submit(request: Request) -> NoReturn:
    ...
    async with sessionmaker() as session:
        ...
        for content, _ in chunk_messages(
            [
                PlainItem(format("**Ban Appeal from** {!m}:\n\n", user_id)),
                PlainItem("**Type:** {}\n".format(kind)),
                PlainItem("**Reason:** "),
                PlainItem(reason),
                PlainItem("\n**Appeal:** "),
            ...
            last_message = await retry(
                lambda: channel.send(content, allowed_mentions=AllowedMentions.none()), attempts=10
            )
        assert last_message
        ...
        embeds = plugins.tickets.summarise_tickets(
            await plugins.tickets.visible_tickets(session, user_id), "Tickets for {}".format(user_id), dm=False
        )
        if embeds:
            embeds = list(embeds)
...
async def init() -> None:
    ...
    await util.db.init(util.db.get_ddl(CreateSchema("appeals"), registry.metadata.create_all))
...
async def close_appeal(interaction: Interaction, appeal_id: int) -> None:
    async with sessionmaker() as session:
        ...
        if msg := await appeal.get_message():
            try:
                await retry(
                    lambda: msg.edit(content="Appeal handled (user may create new ones).", view=None), attempts=10
                )

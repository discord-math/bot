...
async def init() -> None:
    ...
    await util.db.init(util.db.get_ddl(CreateSchema("role_reactions"), registry.metadata.create_all))
...
class RoleReactions(Cog):
    ...
    async def on_raw_reaction_add(self, payload: RawReactionActionEvent) -> None:
        if (member := payload.member) is None:
            return
        if member.bot:
            return
    ...
    async def on_raw_reaction_remove(self, payload: RawReactionActionEvent) -> None:
        ...
        if payload.message_id not in reaction_messages:
            return
        if (guild := client.get_guild(payload.guild_id)) is None:
            return
    ...
    async def rolereact_show(self, ctx: Context, message: Optional[ReplyConverter]) -> None:
        ...
        async with sessionmaker() as session:
            if obj := await session.get(ReactionMessage, msg.id):
                await ctx.send(
                    "Role reactions on {} include: {}".format(
                        msg.jump_url,
                        "; ".join(
    ...
    async def rolereact_add(
        self, ctx: Context, message: ReplyConverter, emoji: Union[PartialEmoji, str], role: PartialRoleConverter
    ) -> None:
        ...
        async with sessionmaker() as session:
            ...
            if reaction := await session.get(Reaction, (message.id, emoji_str)):
                await ctx.send(
                    "Emoji {} already sets role {}".format(
            ...
            await ctx.send(
                "Reacting with {} on message {} now sets {}".format(
    ...
    async def rolereact_remove(self, ctx: Context, message: ReplyConverter, emoji: Union[PartialEmoji, str]) -> None:
        ...
        async with sessionmaker() as session:
            ...
            if not (reaction := await session.get(Reaction, (message.id, emoji_str))):
                await ctx.send(
                    "Role reactions for {} do not exist on {}".format(format_emoji(emoji_str), message.jump_url)
                )
                return
            ...
            await ctx.send(
                "Reacting with {} on message {} no longer sets roles".format(format_emoji(emoji_str), message.jump_url),
                allowed_mentions=AllowedMentions.none(),
            )

...
class RoleOverride(Cog):
    ...
    async def on_member_update(self, before: Member, after: Member) -> None:
        ...
        async with sessionmaker() as session:
            stmt = (
                select(Override.excluded_role_id)
                .distinct()
                .where(Override.retained_role_id.in_([role.id for role in after.roles]))
            )
...
async def config(ctx: Context) -> None:
    ...
    await ctx.send(
        "\n".join(
            format("- having {!M} removes {}", retained, ", ".join(format("{!M}", excluded) for excluded in excludeds))
            for retained, excludeds in overrides.items()
        )
        or "No roles registered",
